...
/*
목표 : 방향키 로직 만들기
    1. 버튼을 눌렀을때, 때었을때 함수를 만든다.
    2. 버튼의 OnClick은 Down+Up이 한 세트이기 때문에 두개의 함수를 동시에 적용하면 싱크가 맞지 않는다.
    3. 이벤트 트리거 컴포넌트를 통해서 함수를 호출한다.
    4. 플레이어 스크립트에 방향버튼을 위한 변수를 만든다.
    5. 함수가 호출될 때 switch문으로 버튼에 따라 변수 값을 채워준다.
    6. 움직이는 힘을 지정하는 로직에 변수 값을 더해 준다.
*/

/*
목표 : 액션키 로직 만들기
    1. 버튼 다운 함수에서 게임 매니저의 함수 호출
    2. 버튼 다운 함수에서 게임 매니저의 함수 호출
*/